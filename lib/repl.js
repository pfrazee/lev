...
module.exports = function(db, config, cache) {
  ...
  repl.context.LS = function(opts) {
    ...
    if (opts.f || opts.filter) {
      let f = opts.f || opts.filter
      ;(Array.isArray(f) ? f : [f]).forEach(filter => {
        filter = filter.split('=')
        filters.push({key: filter[0], value: filter[1]})
      })
      if (filters.length > 0) {
        opts.values = true // must get values if a filter is set
      }
    }

    var fields = []
    if (opts.s || opts.select) {
      let s = opts.s || opts.select
      fields = (Array.isArray(s) ? s : [s])
      opts.values = true // must get values if a 'select' is set
    }
      ...
      if (cache.data.length > 0) {
        ...
        if (opts.values) {
          data = cache.data.map(entry => ({
            key: entry.key,
            value: filterFields(tryParse(entry.value), fields)
          }))
          if (filters.length) {
            data = data.filter(entry => {
              for (var i = 0; i < filters.length; i++) {
                if (entry.value[filters[i].key] != filters[i].value) {
                  return false
                }
              }
              return true
        ...
        process.stdout.write(prettyoutput(data, {maxDepth: 100}))
      }
      else {
        print(null, 'no keys');
      }
...
function filterFields (obj, fields) {
  if (!fields || !fields.length || !obj || typeof obj !== 'object') {
    return obj
  }
  var newObj = {}
